...
|---|---|
| Product | Approved `defaults` (the profile ships only `CountertopColor: Pietra Di Savoia Antracite TQ6`, the material the scene's Top_Solid countertop is authored in; the ULH cabinet is authored in `Antracite Matte OCF`, which configurator 4 does not offer, so no cabinet colour is set); the height and handle of each model and the Multi-Level layout (the compositions came from the team, not the map, §15.7); which Solid-Surface (Mineralmarmo or Ocritech) Low Height uses; the 18 colours missing from configurator 4; the colour → price column map, including the separate `White GL/MT` column (§15.5); the groove colour structure (§15.4); confirmation whether `ProductID = USTD` is intentional (§15.2). |
