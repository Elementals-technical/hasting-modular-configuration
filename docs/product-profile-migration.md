...
- **C06 beyond the handle.** Drawers (including the handle change they trigger in `CabinetBuilderPage`), width and depth, colours and the accessory steps still write state and call the scene directly. Dimensions also need I04, because the scene decides the actual size. The runtime bindings reach the app through a temporary loader until A07 (see the runtime bindings document).
...
C01, C02, C05, C07 and C08 are complete against their acceptance criteria. C06 is in progress: the handle runs through the command service in the app, with its dialog, one owner in state and one send to the scene; the other attributes depend on I04 and B06/B08/B09; C09 depends on A07 and I05. C12 and C13 depend on all of the above.
