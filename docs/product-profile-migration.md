...
`CabinetColor` is recorded with the material and finish the fluting and grain rules read: `resolveColorTraits` finds the variant in `catalog.configurator` the way the colour pages build their options and reads the tokens from `ruleData.cabinetColorTraits`. A colour that cannot be read (no traits section, no configurator section, configurator not loaded) keeps the recorded material instead of wiping it. A groove in the cabinet colour follows it in the same set (`cabinetColor.grooveFollows`); a non-empty groove is written into `selectedProductConfig`, and the presets get the colours of the set in one write. The SKUs are not recorded by the service: they belong to D, and until D02 derives them the colour pages keep writing them, so B moves the colour fields after D02.

Fields get their handler from `useAttributeChangeHandler(attributeId)`. `onChange(value)` addresses the value at the profile scope through `resolveChangeRequest` — a cabinet-scoped value the pages apply to every cabinet (fluting, grain, groove) names the first cabinet, and the binding decides which products the scene updates — records a history step only for a change about to be applied, and returns `preview` with `onConfirm`/`onCancel`, or a `notice` for a refused change. Fluting, grain, towel bar and thickness can switch now; the page's own `setConfigBatch` and `dispatch` go with the switch (B06).

Fluting and grain that become unavailable are cleared with one owner per side: the option listeners in `app/store/optionsListener.ts` clear the stored value, and `useAvailabilityResets` (mounted in `HomePage`) clears the scene through the service whenever availability or the placed cabinets change. The fluting binding sends a cleared value as `"None"` (`emptyValue`), and a declared `initialValue` (`""`) is a catalog member, so a clearing change passes validation. The reset effects of both Color pages still send the same clearing; B removes them with B06.
...
- **C06 beyond handle and drawers.** Width and depth are not planned by the command service yet; the actual size of each cabinet is now recorded from the scene (I04). Fluting, grain, towel bar, thickness and the colours are ready in the command service, and `useAttributeChangeHandler` is the handler for their fields; the fields still write state and call the scene directly until B06 switches them. The colour SKUs stay with the pages until D02 derives them.
...
C01, C02, C05, C07 and C08 are complete against their acceptance criteria. C06 is in progress: the handle and the drawers run through the command service in the app, with one owner in state and one send to the scene; fluting, grain, towel bar, thickness and the colours are accepted and recorded by the service, the cabinet colour with its material, finish, following groove and presets; fields get `useAttributeChangeHandler`, and the scene side of the fluting and grain resets has one owner (`useAvailabilityResets`); the remaining fields depend on B06/B08/B09, the colour SKUs on D02, and the dimensions on I04; C09 is complete in code and unit tests: both saved-configuration restores run through one orchestrator over the I05 scene restorer; `uiState` options are still applied by each page until their fields move to the command service, and the browser check is part of C12. C12 is assembled in [c12-state-acceptance.md](c12-state-acceptance.md): the automated evidence is in place — the five save entry points build one order, a configuration survives save → read → restore with every value addressed to the product and drawer it was saved for, and a test profile refuses a forbidden value and saves its own collection id — and what remains is the browser run on USH, done together with I06. C13 depends on all of the above.
