export function setVisibleDrawerButtons(isEnabled: boolean) {
  ...
  const setVisibleDrawerButtons = canvasIframe?.ConfiguratorAPI?.setVisibleDrawerButtons;
  ...
  if (!setVisibleDrawerButtons) {
    console.warn("[PlayCanvas] ConfiguratorAPI.setVisibleDrawerButtons not ready");
    return null;
  }
  ...
  try {
    ...
    console.error("[PlayCanvas] Failed to set setVisibleDrawerButtons", error);
    return null;
  }
}
