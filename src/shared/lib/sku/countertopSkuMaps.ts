/** Countertop Style → SKU code */
export const countertopStyleSkuMap: Record<string, string> = {
  plain: "X",
  integrated: "INTG",
  vessel: "VES",
  undermount: "UDMT",
};

/** Material name → MaterialSKU */
export const countertopMaterialSkuMap: Record<string, string> = {
  Fenix: "FX",
  HPL: "HPL",
  Porcelain: "POR",
  "Glass Matt": "GLSM",
  "Glass Gloss": "GLSG",
  Minermalmaro: "SSMLM",
  Teckormud: "SSTM",
  Ocritech: "SSOCR",
  Tekorlux: "SSTKR",
};

/** PlayCanvas sinkType → Basin SKU code */
export const basinSkuMap: Record<string, string> = {
  Top_HPLPrisma: "PRISMA",
  Top_HPLQuadra: "QUADRA",
  Top_HPLCover: "COVER",
  Top_HPLStrip: "STRIP48",
  "Top_HPL/Fenix_Cover_Gres": "COVER",
  "Top_HPL/Fenix_Prisma_Gres": "PRISMA",
  "Top_HPL/Fenix_Quadra_Gres": "QUADRA",
  "Top_HPL/Fenix_Strip_Gres": "STRIP48",
  Top_Glass_Ovale: "OVALE",
  Top_Mineralmarmo_Diamond: "DIAMOND",
  Top_Ocritech_Oly55: "OLY55",
  Top_Ocritech_Oly56: "OLY56",
  Top_Ocritech_Orion: "ORION",
  Top_Ocritech_Quadra: "QUADRA",
  Top_Ocritech_Rayo: "RAYO",
  Top_Ocritech_Roll: "ROLL",
  Top_Porcelain_Cover: "COVER",
  Top_Porcelain_Prisma: "PRISMA",
  Top_Porcelain_Quadra: "QUADRA",
  Top_Porcelain_Strip: "STRIP48",
  Top_Syntesi: "SYNT48",
  Top_Tekorlux_Quadra: "QUADRA",
  Top_Tekorlux_Rectangular: "RECT50",
  Top_Tekorlux_Trip: "TRIP50",
  Top_Tekorlux_Ron: "RON50",
  Top_Tekormud_Tivi: "TIVI",
};
