...
describe("partial production collection packages", () => {
  ...
  it("validates the Urban Low Height collection-specific UI contract", () => {
    const result = validateCustomizationSchema(urbanLowHeightUi);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.schema.collectionId).toBe("urban-low-height");
  });
});
