...
- [ ] `fixture-ui` provides a manifest, ProductProfile, customization schema, and runtime bindings through test injection.
- [ ] Its customization schema exposes the additional test finish field and option through active collection data.
- [ ] Its runtime binding resolves `TestGrooveFinish` to the expected `HandleGrooveColor` scene key or patch.
